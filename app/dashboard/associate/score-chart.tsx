...
export default function ScoreChart({ data }: { data: Point[] }) {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-[#111827] p-6">
      <h2 className="mb-4 text-sm font-semibold text-[#2dd4bf]">Score Trend</h2>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" aria-hidden>
        ...
        {[0, 25, 50, 75, 100].map(tick => (
          <g key={tick}>
            ...
            />
            <text x={PAD.left - 6} y={yAt(tick)} textAnchor="end" dominantBaseline="middle" fontSize={9} fill="rgba(255,255,255,0.35)">
              {tick}
            </text>
          </g>
        ))}
        ...
        {pts.map((p, i) => (
          <g key={i}>
            <circle cx={p.cx} cy={p.cy} r={4} fill="#2dd4bf" />
            <text x={p.cx} y={H - PAD.bottom + 14} textAnchor="middle" fontSize={9} fill="rgba(255,255,255,0.35)">
              {new Date(p.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
            </text>
            <text x={p.cx} y={p.cy - 8} textAnchor="middle" fontSize={9} fontWeight="600" fill="rgba(255,255,255,0.8)">
              {p.score}
            </text>
          </g>
        ))}
