...
export default function SignUpPage() {
  ...
  return (
    <>
      <h1 className="mb-8 text-2xl font-semibold tracking-tight text-zinc-900">Create an account</h1>

      {state?.message && (
        <p className="mb-4 rounded-md bg-green-50 px-4 py-3 text-sm text-green-700">{state.message}</p>
      )}
      {state?.error && (
      ...
      <p className="mt-6 text-center text-sm text-zinc-500">
        ...
        <Link href="/sign-in" className="font-medium text-zinc-900 hover:underline">Sign in</Link>
      </p>
    </>
  )
}
